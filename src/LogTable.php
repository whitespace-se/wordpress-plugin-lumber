...
class LogTable extends WP_List_Table {
  ...
  protected function _get_where_by_role() {
    ...
    foreach ($this->_roles as $key => $role) {
      if (
        ...
        current_user_can("view_all_aryo_activity_log")
      ) {
  ...
  public function __construct($args = []) {
    ...
    $this->_roles = apply_filters("aal_init_roles", [
      // admin
      "manage_options" => [
        "Core",
        "Export",
    ...
    $this->_caps = apply_filters("aal_init_caps", [
  ...
  public function column_default($item, $column_name) {
    ...
    switch ($column_name) {
      case "action":
        $return =
          '<a href="' .
          $this->get_filtered_link("showaction", $item->action) .
          '">' .
          $this->get_action_label($item->action) .
          "</a>";
        break;
      ...
      case "date":
        ...
        $return .=
          '<br /><a href="' .
          $this->get_filtered_link("dateshow", $date_formatted) .
          '">' .
      ...
      case "ip":
        $return =
          '<a href="' .
          $this->get_filtered_link("filter_ip", $item->hist_ip) .
          '">' .
          $item->hist_ip .
          "</a>";
        break;
    ...
    switch ($item->object_type) {
      ...
      case "Users":
        ...
        if (!empty($item->object_name)) {
          $return =
            __("Username:", "aryo-activity-log") . " " . $item->object_name;
        }
        break;
  ...
  public function extra_tablenav_footer() {
     ...
     */
    $actions = apply_filters("aal_record_actions", []); ?>
  ...
  public function extra_tablenav($which) {
    ...
    $filters = ["dateshow", "capshow", "usershow", "typeshow", "showaction"];

    foreach ($filters as $filter) {
      if (!empty($_REQUEST[$filter])) {
        echo '<a href="' .
          $this->get_filtered_link() .
          '" id="aal-reset-filter"><span class="dashicons dashicons-dismiss"></span>' .
  ...
  public function prepare_items() {
    ...
    if (
      ...
      !in_array($_REQUEST["order"], ["desc", "asc"])
    ) {
      $_REQUEST["order"] = "DESC";
    }
    if (
      !isset($_REQUEST["orderby"]) ||
      !in_array($_REQUEST["orderby"], ["hist_time", "hist_ip"])
    ) {
    ...
    if (isset($_REQUEST["dateshow"])) {
      ...
      if (
        in_array($_REQUEST["dateshow"], ["today", "yesterday", "week", "month"])
      ) {
        // Today
        $start_time = mktime(
          0,
